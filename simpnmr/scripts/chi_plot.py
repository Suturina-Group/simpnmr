...
import numpy as np
from scipy.constants import Avogadro, c, h, k, mu_0, physical_constants
    ...
    try:
        ...
        if fixed_intercept is not None:
            ...
            if np.allclose(w, w[0]):
                ...
                Cov = ATA_inv
                if dof > 0 and not (
                    sigma is not None and any(s is not None for s in sigma)
                ):
                    s2 = RSS_w / dof
                    Cov = Cov * s2
...
def _finalize_axes(
    ...
    sec_ax = ax.secondary_xaxis(
        "top",
        functions=(
            lambda inv: np.divide(
                1, inv, out=np.full_like(inv, np.nan), where=inv != 0
            ),
...
def _plot_component(
    ...
    if len(inv_t_csv) == len(fit_csv) and len(inv_t_csv) > 0:
        ...
        if (
            sdev_csv is not None
            and len(sdev_csv) == len(inv_t_csv)
            and any(v is not None for v in sdev_csv)
        ):
...
def _run_lr_for_components(
    ...
    chi_iso_fit_pred, a_iso, b_iso, chi_iso_sdev_pred, a_iso_se, b_iso_se = (
    ...
    chi_ax_fit_pred, a_ax, b_ax, chi_ax_sdev_pred, a_ax_se, b_ax_se = (
        weighted_linreg_predict(
            inv_t,
            chi_ax,
            inv_t,
    ...
    chi_rho_fit_pred, a_rho, b_rho, chi_rho_sdev_pred, a_rho_se, b_rho_se = (
        weighted_linreg_predict(
            inv_t,
            chi_rho,
            inv_t,
...
def _compute_analytic_components(temps, g_sq_iso, g_sq_ax, g_sq_rh, D_J, E_J, f_S):
    ...
    chi_ax_analytic = [
        g_sq_ax[i]
        - (f_S / (30 * k * temps[i]))
        * ((D_J[i]) * (g_sq_ax[i] + 3 * g_sq_iso[i]) - 3 * E_J[i] * g_sq_rh[i])
        for i in range(len(temps))
    ]
    ...
    chi_rho_analytic = [
        g_sq_rh[i]
        + (f_S / (30 * k * temps[i]))
        * (E_J[i] * (g_sq_ax[i] - 3 * g_sq_iso[i]) + D_J[i] * g_sq_rh[i])
        for i in range(len(temps))
    ]
...
def _load_orca_data(file_name: str, section: str, t_limit: float = 248.0):
    """Read ORCA chi tensors, spin, g-tensor and Effective Hamiltonian, filtered by temperature."""
    try:
...
def _normalize_csv(comp_csv: ComponentData, norm_factor: float):
    for name in ("iso", "ax", "rho"):
        if comp_csv.get_series(name):
            comp_csv[name]["series"] = _normalize_list(
                comp_csv.get_series(name), norm_factor
            )  # type: ignore[arg-type]
        if comp_csv[name].get("sdev_raw"):
            comp_csv[name]["sdev"] = _normalize_list(
                comp_csv[name].get("sdev_raw"), norm_factor
            )  # type: ignore[arg-type]


def _read_csv_data(
    csv_path: str | None, t_limit: float
) -> tuple[ComponentData, list[float]]:
...
def _lr_with_optional_csv(
    ...
    if csv_path is not None and len(inv_t_csv) >= 2:
        ...
        chi_iso_fit_pred, a_iso, b_iso, chi_iso_sdev_pred, a_iso_se, b_iso_se = lr[
            "iso"
        ]
        chi_ax_fit_pred, a_ax, b_ax, chi_ax_sdev_pred, a_ax_se, b_ax_se = lr["ax"]
        chi_rho_fit_pred, a_rho, b_rho, chi_rho_sdev_pred, a_rho_se, b_rho_se = lr[
            "rho"
        ]
        ...
        if csv_path is None:
            ...
            ut.cprint(
                "Not enough CSV points for linear regression (need at least 2).", "cyan"
            )
...
def _recover_gD_from_lr(lr, chi_only: bool = False, spin_S: float | None = None):
    ...
    if (
        not chi_only
        and (a_ax_TIP is not None)
        and (b_ax_TIP is not None)
        and (b_iso_TIP is not None)
    ):
        try:
            use_axial_only = (a_rho is None) or (abs(a_rho) < 1e-6) or (b_rho is None)
            if use_axial_only:
                gx_TIP, gy_TIP, gz_TIP = solve_g_principals_from_LR(
                    b_iso=b_iso_TIP, b_ax=b_ax_TIP, b_rh=None
                )
                g_sq_iso_analytic_TIP_val = calculate_g_sq_iso_analytic(
                    b_iso_intercept=b_iso_TIP, b_ax_intercept=b_ax_TIP
                )
            else:
                gx_TIP, gy_TIP, gz_TIP = solve_g_principals_from_LR(
                    b_iso=b_iso_TIP, b_ax=b_ax_TIP, b_rh=b_rho
                )
                g_sq_iso_analytic_TIP_val = (gx_TIP**2 + gy_TIP**2 + gz_TIP**2) / 3.0
            D_J_LR_TIP, E_J_LR_TIP = solve_D_E_from_LR(
                a_ax=a_ax_TIP, a_rh=a_rho, S=spin_S, gx=gx_TIP, gy=gy_TIP, gz=gz_TIP
            )  # type: ignore[arg-type] # noqa
    ...
    return {
        ...
        "gD_results": (
            ...
            D_from_LR,
            E_from_LR,
            g_sq_iso_analytic_val,
            gx_TIP,
            gy_TIP,
            gz_TIP,
            D_from_LR_TIP,
            E_from_LR_TIP,
            g_sq_iso_analytic_TIP_val,
        ),
    }
...
def calculate_chi_components_nevpt2(
    tensors: dict[float, np.ndarray], g_matrix: np.ndarray
    ...
    for T in tensors.keys():
        ...
        chi_rho.append(
            (chi_diag_traceless[0, 0] - chi_diag_traceless[1, 1])
            / 2.0
            / Avogadro
            * conv
        )
        ...
        term = np.divide(
            chi_diag_scaled,
            g_diag,
            out=np.zeros_like(chi_diag_scaled, dtype=float),
            where=g_diag != 0,
        )
...
def calculate_g_components(
    rotated_g_tensors: list[np.ndarray],
...
def calculate_E_D_components(
    rotated_eff_H_tensors: list[np.ndarray],
    ...
    with open(output_csv_path, "w", newline="") as f:
        ...
        for i in range(min_len):
            row = [
                analysis.temps[i],
                analysis.inv_t[i],
                analysis.chi_iso_nevpt2_si[i],
                analysis.chi_ax_nevpt2_si[i],
                analysis.chi_rho_nevpt2_si[i],
                analysis.chi_iso_analytic[i],
                analysis.chi_ax_analytic[i],
                analysis.chi_rho_analytic[i],
                analysis.g_sq_iso[i],
                analysis.g_sq_ax[i],
                analysis.g_sq_rh[i],
                analysis.D_list[i],
                ...
                analysis.D_J[i],
                analysis.E_J[i],
                _get_opt(analysis.chi_iso_fit_pred, i),
                _get_opt(analysis.chi_ax_fit_pred, i),
                _get_opt(analysis.chi_rho_fit_pred, i),
                _get_opt(analysis.chi_iso_sdev_pred, i),
                _get_opt(analysis.chi_ax_sdev_pred, i),
                _get_opt(analysis.chi_rho_sdev_pred, i),
            ]
            writer.writerow(row)
        ...
        if (
            analysis.temps_csv
            and analysis.inv_t_csv
            and analysis.chi_iso_fit_csv
            and analysis.chi_ax_fit_csv
            and analysis.chi_rho_fit_csv
        ):
            # Blank separator row
            writer.writerow([])
            ...
            for j in range(n_raw):
                iso_sd = (
                    ""
                    if (
                        analysis.chi_iso_sdev_csv is None
                        or j >= len(analysis.chi_iso_sdev_csv)
                        or analysis.chi_iso_sdev_csv[j] is None
                    )
                    else analysis.chi_iso_sdev_csv[j]
                )
                ax_sd = (
                    ""
                    if (
                        analysis.chi_ax_sdev_csv is None
                        or j >= len(analysis.chi_ax_sdev_csv)
                        or analysis.chi_ax_sdev_csv[j] is None
                    )
                    else analysis.chi_ax_sdev_csv[j]
                )
                rho_sd = (
                    ""
                    if (
                        analysis.chi_rho_sdev_csv is None
                        or j >= len(analysis.chi_rho_sdev_csv)
                        or analysis.chi_rho_sdev_csv[j] is None
                    )
                    else analysis.chi_rho_sdev_csv[j]
                )
                writer.writerow(
                    [
                        analysis.temps_csv[j],
                        analysis.inv_t_csv[j],
                        analysis.chi_iso_fit_csv[j],
                        analysis.chi_ax_fit_csv[j],
                        analysis.chi_rho_fit_csv[j],
                        iso_sd,
                        ax_sd,
                        rho_sd,
                    ]
...
def read_susceptibility_csv(
    ...
    if temp_col is None:
        raise KeyError(
            f"Column 'Temperature (K)' not found. Detected headers: {detected_header}"
        )
    ...
    for row in reader:
        ...
        try:
            ...
            if unit_iso == "cm^3 mol^-1":
                ...
                logging.warning(
                    f"Unrecognized susceptibility unit '{unit_iso}'. Assuming cm^3 mol^-1."
                )
                to_cm3_per_mol = 1.0
        ...
        if iso_sdev_col is not None:
            try:
                sdev_raw = (
                    float(str(row.get(iso_sdev_col, "")).strip()) * to_cm3_per_mol
                )
        ...
        if ax_sdev_col is not None:
            try:
                sdev_raw_ax = (
                    float(str(row.get(ax_sdev_col, "")).strip()) * to_cm3_per_mol
                )
                # Convert the std dev of chi to std dev of chi*T with the same scaling
        ...
        if rho_sdev_col is not None:
            try:
                sdev_raw_rho = (
                    float(str(row.get(rho_sdev_col, "")).strip()) * to_cm3_per_mol
                )
...
def compute_chi_temperature_dependence(
    ...
    if section is None:
        ...
        if n_pts == 1:
            ...
            if (a_ax is not None) and (b_ax is not None) and (b_iso is not None):
                try:
                    use_axial_only = (
                        (b_rho is None) or (abs(b_rho) < 1e-6) or (a_rho is None)
                    )
                    ...
                    if g_sq_iso_analytic_val is not None:
                        ...
                        g_sq_ax = (
                            [g_sq_ax_val] * n_pts if g_sq_ax_val is not None else []
                        )
                        g_sq_rh = (
                            [g_sq_rh_val] * n_pts if g_sq_rh_val is not None else []
                        )
                    else:
                        g_sq_iso = g_sq_ax = g_sq_rh = []
        ...
        print(f"slope iso:{a_iso}, slope ax:{a_ax}, slope rho:{a_rho}")
    ...
    temps, tensors, S, g_matrix, eff_H_raw = _load_orca_data(
        file_name, section, t_limit
    )

    chi_iso_nevpt2_si, chi_ax_nevpt2_si, chi_rho_nevpt2_si, chi_eigenvectors = (
        calculate_chi_components_nevpt2(tensors, g_matrix)
    )
    ...
    _normalize_theory_chit(
        chi_iso_nevpt2_si, chi_ax_nevpt2_si, chi_rho_nevpt2_si, norm_factor
    )
    ...
    if temps:
        try:
            ...
            chi_iso_red_TIP = (
                chi_iso_nevpt2_chi[idx_ref] - chi_iso_analytic_chi[idx_ref]
            )
            ut.cprint(
    ...
    try:
        if (
            (gx_noTIP is not None)
            and (gz_noTIP is not None)
            and (D_from_LR is not None)
        ):
            if E_from_LR is not None:
                ut.cprint(
        ...
        if (
            (gx_TIP is not None)
            and (gz_TIP is not None)
            and (D_from_LR_TIP is not None)
        ):
    ...
    try:
        ...
        if (
            ...
            def _lr_branch(
                chi_iso_series, chi_ax_series, chi_rho_series, use_fixed: bool
            ):
            ...
            (
                ...
                chi_iso_minus, chi_ax_minus, chi_rho_minus, use_fixed_intercept
            )
            ...
            def _params_from_LR(a_ax_loc, b_ax_loc, a_rho_loc, b_rho_loc, b_iso_loc):
                ...
                use_axial_only_loc = (
                    (b_rho_loc is None)
                    or (abs(b_rho_loc) < 1e-6)
                    or (a_rho_loc is None)
                )
...
def plot_chi_temperature_dependence(
    ...
    for comp, color, outfile in comps:
        _plot_component(
            ...
            getattr(
                analysis,
                f"g_sq_{'ax' if comp == 'ax' else ('rh' if comp == 'rho' else 'iso')}",
            ),
            analysis.inv_t_csv,
    ...
    if (
        ...
        for comp, color in (("iso", "blue"), ("ax", "green"), ("rho", "red")):
            ...
            if len(sdev) == len(analysis.inv_t_csv) and any(
                v is not None for v in sdev
            ):
...
def main():
    ...
    parser.add_argument("susc_file", help="ORCA output file with susceptibility data")
    parser.add_argument(
        "section",
        nargs="?",
    ...
    parser.add_argument(
        "csv_file", nargs="?", default=None, help="Optional additional CSV file to read"
    )
    parser.add_argument(
        "--log-level",
    ...
    try:
        ...
        if args.section is None:
            ...
            if args.spin is None:
                parser.error(
                    "CSV-only mode detected (no 'section' argument). Please provide --spin S (e.g. --spin 2.0)."
                )
            ...
            ut.cprint(
                "Saved plots: chi_plot_iso.png, chi_plot_ax.png, chi_plot_rho.png, chi_plot_all.png",
                "cyan",
            )
        else:
