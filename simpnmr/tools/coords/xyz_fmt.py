...
def num_to_lab(numbers: ArrayLike, numbered: bool = True) -> list[str]:
    ...
    labels = [ptable.num_lab[int(num)] for num in numbers]
...
def lab_to_num(labels: ArrayLike | str) -> list[int]:
    ...
    numbers = [ptable.lab_num[lab] for lab in labels_nn]
...
def check_xyz(
    ...
    if not allow_nonelements:
        if any([lab not in ptable.elements for lab in _labels_nn]):
